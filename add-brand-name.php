	   ...
	   ?> 	   
			   ...
			   <label>License</label>
						...
						<option value=""> Select License</option>
						 <?php 
                              foreach($licence->list as $lic){
								  ?>
								  <option value="<?php echo $lic->licenseId;?>"><?php echo $lic->licenseName;?> </option>
								  <?php
							  }						 
			    ...
			    <div class="col-md-6 add-form" >
				   ...
				   <textarea class="form-control" rows="2" name="description" id="description" style="resize:none;"></textarea>
			   ...
			   <button type="submit" class="add-btn add-margin btn-primary" name="create" id="create" >Create</button>
			   </div>
	...
	if(description.length < 1){
			...
			return false;
				...
				$.ajax({
					 ...
					 url:"ajax.php",
					 dataType:'json',
					 data:{'brand_name':brand_name,'brand_code':brand_code,'internal_brand_code':internal_brand_code,'principle':principle,'license':license,'brand_owner':brand_owner,'description':description,'state':state,'distributerid':distributerId,'page':"add_brand"},
					cache: false,
  ...
  <table class="table table-hover" id="example" style="border:1px solid #ddd; font-size:12px;">
    <thead>
      <tr>
        ...
        <th>Description</th>
          <?php
		  if($_SESSION['userData']['roleId']=='1' || $rowdatapermissionValid->list[23]->edit=='Y'){
			  ?>
			  <th>Action</th>
			  <?php
		  }
	...
	?>
	    <tr>
			...
			?>
			<td> 
			<a href="edit-brandname.php?id=<?php echo $value->brandId;?>">
