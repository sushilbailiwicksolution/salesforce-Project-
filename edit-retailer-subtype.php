      ...
      <ol class="breadcrumb">
	   ...
	   <div class="container">
		   ...
		   <div class="row" style="margin: 18px -15px;">
			   ...
			   <?php
			    foreach($rowdata->lstRetailerDetailedData as $retailerSubtype){
					if($retailerSubtype->id==$_REQUEST['id']){
						?>
					     <label>Retailer Sub Type</label>
				<font color='red'> <span id="name_error" style='float:right;' ></span></font>
				<input type="text" name="name" id="retailer_subtype" class="form-control" value="<?php echo $retailerSubtype->name;?>" required>
				   <label>Description</label>
				  <font color='red'> <span id="description_error" style='float:right;' ></span></font>
				  <textarea class="form-control" name="description" id="retailer_description" rows="3" style="resize:none;" required><?php echo $retailerSubtype->description;?></textarea>	
						<?php
...
//						alert("Empty");  	
		...
		function CallAjaxToSave(name,description,id){
				$.ajax({
					...
					url:"ajax.php",
					dataType:'json',
					data:{'name':name,'description':description,'id':id,'page':"update_retailersubtype"},
					cache: false,
					...
					//alert(retailersubtype);
						if(retailersubtype.statusCode == 0) {
							swal("Success",retailersubtype.message, "success");
								setTimeout(function(){
								self.location.href="add_subtype.php";
							}, 2000);
							}else {
