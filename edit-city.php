      ...
      <ol class="breadcrumb">
	   ...
	   <div class="container">
		   ...
		   <div class="row" style="margin: 18px -15px;">
			   ...
			   <?php
			   foreach($rowdata->lstCommnodetail as $city_details){
				   
			   
			   if($_REQUEST['city_id']==$city_details->id){
				   ?>
				     <label>Add City</label>
				 <font color='red'> <span id="name_error" style='float:right;' ></span></font>
				 <input type="text" class="form-control" name="City_name" id="CityName" required value=<?php echo $city_details->name;?>> 
				   <?php
			   ...
			   ?>
				  ...
				  <!-- <input type="date" class="form-control" name="createdate" id="retailer_createdate">-->
					...
					<button class="add-btn add-margin" name="create" id="update_city">Update</button>
...
<div class="row">	   
  <table class="table table-hover" style="border:1px solid #ddd; font-size:12px;">
	...
	<script>
		...
		$('#update_city').on('click',function(){
			let city_name=$('#CityName').val();
			let city_id='<?php echo $_REQUEST['city_id'];?>';
		
			$.ajax({
			type:'POST',
			url:'ajax.php',
			data:{'City_name':city_name,'city_id':city_id,'page':'update_city'},
			cache:false,
