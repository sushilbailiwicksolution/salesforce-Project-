...
$('#role').on('change', function () {
	...
	if(roleid!=''){
			$.ajax({
				type: "POST",
				url: 'ajax.php',
				data: 'roleid='+roleid+'&distributerid='+distributerId+'&page=getReport',
...
$("#email_id").fadeIn("fast", function() { $(tlhis).delay(5000).fadeOut("slow"); });
		...
		$("#manager_team").click(function(){			
			...
			//alert(distributerid);
				$.ajax({
					 ...
					 url:"ajax.php",
					 dataType:'json',
					 data:{'fname':fname,'lname':lname,'email':email,'phone':phone,'mobile':mobile,'address':address,'emp_id':emp_id,'doj':doj,'role':role,'designation':designation,'state':state,'report':report,'password':password,'distributerid':distributerId,'page':"create_team"},
					cache: false,
